...
def build_intermediate_prefix(ppt_path: Path) -> str:
    """Return a per-presentation prefix for intermediate slide XML files."""
    suffix = ppt_path.suffix.lower().lstrip(".")
    return f"{ppt_path.stem}_{suffix}"
...
def ppt_to_xml(
    ...
    *,
    ...
    target_lang: str,
    max_workers: int = 4,
    intermediate_prefix: str | None = None,
) -> Optional[str]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future, slide_number in future_to_slide.items():
                ...
                filename = (
                    f"{intermediate_prefix}_slide_{slide_number}_{'translated' if translator else 'original'}.xml"
                    if intermediate_prefix
                    else f"slide_{slide_number}_{'translated' if translator else 'original'}.xml"
                )
                intermediate_path = base_dir / filename
...
def create_translated_ppt(original_ppt_path: str, translated_xml_path: str, output_ppt_path: str) -> bool:
...
def process_ppt_file(
    ...
    *,
    ...
    intermediate_prefix = build_intermediate_prefix(ppt_path)
    ...
    try:
        ...
        if not create_translated_ppt(str(ppt_path), str(translated_output_path), str(output_ppt_path)):
            return None
        ...
        if cleanup:
            cleanup_intermediate_files(base_dir, pattern=f"{intermediate_prefix}_slide_*.xml")
            cleanup_intermediate_files(base_dir, pattern=original_output_path.name)
            cleanup_intermediate_files(base_dir, pattern=translated_output_path.name)
            if not success:
                cleanup_intermediate_files(base_dir, pattern=output_ppt_path.name)
